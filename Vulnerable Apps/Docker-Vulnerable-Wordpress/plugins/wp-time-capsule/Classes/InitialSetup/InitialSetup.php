...
namespace Docker-Vulnerable - Wordpress\plugins\wp - time - capsule\Classes\InitialSetup;

use Dockeruse Dockeruse Dockeruse Dockeruse Dockeruse Dockerclass Wptc_InitialSetup extends Wptc_InitialSetup_Init {
        public
        $tabs;
        private
        $config,
	...
	public function __construct()
        {
            $this->config = WPTC_Base_Factory::get('Docker-Vulnerable-Wordpress\plugins\wp-time-capsule\Classes\InitialSetup\Wptc_InitialSetup_Config');
	...
	public function is_fresh_backup()
        {
            global $wpdb;
            $fcount = $wpdb->get_results('SELECT COUNT(*) as files FROM ' . $wpdb->base_prefix . 'Docker-Vulnerable-Wordpress\plugins\wp-time-capsule\Classes\Processed\WPTC_Processed_Files');
